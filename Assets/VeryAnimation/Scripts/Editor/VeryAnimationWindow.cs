    ...
    public class VeryAnimationWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            else if (!va.edit)
            {
                ...
                errorLogScrollPosition = EditorGUILayout.BeginScrollView(errorLogScrollPosition);
                {
                    #region UnityVersion
                    {
#if UNITY_2019_1_OR_NEWER || !UNITY_5_6_OR_NEWER
                                    ...
                                    #region Warning
                                    {
                                        ...
                                        if (count > 0)
                                        {
                                            ...
                                            if (guiAnimationWarningFoldout)
                                            {
                                                ...
                                                if (hasMotionCurves)
                                                {
                                                    EditorGUILayout.HelpBox(Language.GetText(Language.Help.AnimationClipSettingsHasMotionCurves), MessageType.Warning);
                                                }
                                                if (!va.animatorApplyRootMotion && hasRootCurves && !hasMotionCurves &&
        ...
        private void CustomUpdate()
        {
            if (initialized)
            {
                ...
                {
                    #region ChangeOtherObject
                    var currentTime = va.currentTime;
                    Release();
                    ...
                    {
                        Initialize();
                        va.uAw.SetCurrentTime(currentTime);
                    }
                    #endregion
                }
            }
