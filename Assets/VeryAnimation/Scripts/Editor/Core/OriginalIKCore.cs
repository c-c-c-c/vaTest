...
namespace VeryAnimation
{
    ...
    public class OriginalIKCore
    {
                ...
                #region SetValue
                {
                    for (int target = 0; target < ikData.Count; target++)
                    {
                        ...
                        if (time == va.currentTime)
                        {
                            if (va.mirrorEnable)
                            {
                                var mtarget = GetMirrorTarget(target);
                                if (mtarget >= 0)
                                {
                                    if (!ikData[mtarget].updateIKtarget)
                                        ikData[mtarget].synchroIKtarget = true;
                                }
                            }
                        }
                        #endregion
        ...
        public void SelectionGUI()
        {
            ...
            #region IK
            {
                ...
                #region Mirror
                {
                    ...
                    if (GUILayout.Button(Language.GetContentFormat(Language.Help.SelectionMirror, (mirrorTarget >= 0 ? string.Format("From 'IK: {0}'", ikData[mirrorTarget].name) : "From self")), GUILayout.Width(100)))
                    {
                        ...
                        if (ikTargetSelect != null)
                        {
                            foreach (var ikTarget in ikTargetSelect)
                            {
                                if (!ikData[ikTarget].enable) continue;
                                ikData[ikTarget].synchroIKtarget = true;
                            }
                        }
                    }
        ...
        public void SetUpdateIKtargetHumanoidIndex(HumanBodyBones humanoidIndex)
        {
            ...
            if (boneIndex < 0)
            {
                var virtualIndex = va.GetHumanVirtualBoneParentBone(humanoidIndex);
                if (virtualIndex >= 0)
                    boneIndex = va.humanoidIndex2boneIndex[(int)virtualIndex];
            }
        ...
        {
            if (target < 0 || ikData == null) return;
            ...
            for (int i = 0; i < ikData[target].level; i++)
            {
                va.SetUpdateIKtargetBone(va.BonesIndexOf(ikData[target].joints[i].bone));
            }
        }
