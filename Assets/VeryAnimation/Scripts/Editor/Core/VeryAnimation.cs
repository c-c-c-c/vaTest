    ...
    public partial class VeryAnimation
    {
        ...
        public void Initialize()
        {
            ...
            if (uAw_2017_1.GetLinkedWithTimeline())
            {
                ...
                if (vaw.editorSettings.settingDummyObjectColorChange)
                                            ...
                                            if (mcurve != null)
                                            {
                                                ...
                                                else if (IsTransformPositionCurveBinding(pair.Value.binding))
                                                {
                                                    ...
                                                    for (int i = 0; i < curve.length; i++)
                                                    {
                                                        if (FindKeyframeIndex(pair.Value.beforeCurve, curve, i) < 0)
                                                        {
                                                            var position = GetAnimationValueTransformPosition(boneIndex, curve[i].time);
                                                            var mposition = GetMirrorBoneLocalPosition(boneIndex, position);
                                                            for (int dof = 0; dof < 3; dof++)
                                                            {
                                                                if (tmpCurves.curves[dof] == null || tmpCurves.subCurves[dof] == null)
                                                                    continue;
                                                                var keyIndex = FindKeyframeAtTime(tmpCurves.subCurves[dof], curve[i].time);
                                                                if (keyIndex >= 0)
                                                                {
                                                                    var key = tmpCurves.subCurves[dof][keyIndex];
                                                                ...
                                                                {
                                                                    SetKeyframe(tmpCurves.curves[dof], curve[i].time, mposition[dof]);
                                                                }
                                                            }
                                                            updated = true;
                                                        }
                                                ...
                                                else if (IsTransformRotationCurveBinding(pair.Value.binding))
                                                {
                                                    ...
                                                    if (mbinding.Value.propertyName.StartsWith(URotationCurveInterpolation.PrefixForInterpolation[(int)URotationCurveInterpolation.Mode.RawQuaternions]))
                                                    {
                                                        ...
                                                        for (int i = 0; i < curve.length; i++)
                                                        {
                                                            if (FindKeyframeIndex(pair.Value.beforeCurve, curve, i) < 0)
                                                            {
                                                                var localRotation = GetAnimationValueTransformRotation(boneIndex, curve[i].time);
                                                                var mlocalRotation = GetMirrorBoneLocalRotation(boneIndex, localRotation);
                                                                mlocalRotation = FixReverseRotationQuaternion(tmpCurves.curves, curve[i].time, mlocalRotation);
                                                                for (int dof = 0; dof < 4; dof++)
                                                                {
                                                                    ...
                                                                    {
                                                                        SetKeyframe(tmpCurves.curves[dof], curve[i].time, mlocalRotation[dof]);
                                                                    }
                                                                }
                                                                updated = true;
                                                            }
                                                    ...
                                                    else if (mbinding.Value.propertyName.StartsWith(URotationCurveInterpolation.PrefixForInterpolation[(int)URotationCurveInterpolation.Mode.RawEuler]))
                                                    {
                                                        ...
                                                        for (int i = 0; i < curve.length; i++)
                                                        {
                                                            if (FindKeyframeIndex(pair.Value.beforeCurve, curve, i) < 0)
                                                            {
                                                                var localRotation = GetAnimationValueTransformRotation(boneIndex, curve[i].time);
                                                                var mlocalRotation = GetMirrorBoneLocalRotation(boneIndex, localRotation);
                                                                var meulerAngles = FixReverseRotationEuler(tmpCurves.curves, curve[i].time, mlocalRotation.eulerAngles);
                                                                for (int dof = 0; dof < 3; dof++)
                                                                {
                                                                    if (tmpCurves.curves[dof] == null || tmpCurves.subCurves[dof] == null)
                                                                        continue;
                                                                    ...
                                                                    {
                                                                        SetKeyframe(tmpCurves.curves[dof], curve[i].time, meulerAngles[dof]);
                                                                    }
                                                                }
                                                                updated = true;
                                                            }
                                                ...
                                                else if (IsTransformScaleCurveBinding(pair.Value.binding))
                                                {
                                                    ...
                                                    for (int i = 0; i < curve.length; i++)
                                                    {
                                                        if (FindKeyframeIndex(pair.Value.beforeCurve, curve, i) < 0)
                                                        {
                                                            var scale = GetAnimationValueTransformScale(boneIndex, curve[i].time);
                                                            ...
                                                            for (int dof = 0; dof < 3; dof++)
                                                            {
                                                                ...
                                                                {
                                                                    SetKeyframe(tmpCurves.curves[dof], curve[i].time, mscale[dof]);
                                                                }
                                                            }
                                                            updated = true;
                                                        }
        ...
        public void PlayableDirectorEvaluate(float time = -1)
        {
            ...
            uAw_2017_1.SetCurrentTime(time);
        ...
        public void AnimationWindowSampleAnimationOverride()
        {
            ...
            if (!uAw_2017_1.GetLinkedWithTimeline())
            {
                if (!transformPoseSave.IsRootStartTransform())
                {
                    ResampleAnimation();
                }
            }
        ...
        private void OnHierarchyWindowChanged()
        {
            ...
            {
                vaw.Release();
                Debug.Log("<color=blue>[Very Animation]</color>Editing ended : OnHierarchyWindowChanged");
                return;
            }
            for (int i = 0; i < bones.Length; i++)
            {
        ...
        {
            bool done = false;
            var bounds = new Bounds(Vector3.zero, gameObjectBounds.size * sizeAdjustment);
